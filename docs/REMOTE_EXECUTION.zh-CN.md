...
如果选择密码认证，先安装 `sshpass`，然后用隐藏输入保存密码并移除 key 配置：
...
`600`。密码模式会在该文件中明文保存密码，但 CLI 输出始终脱敏。密码只在本机
执行 `sshpass -e` 时动态注入，不写入 YAML、`JobSpec`、任务历史或远端文件。

提交作业时 Profile 的 endpoint 会被解析并固化进 `JobSpec`。密码只保留一个本地
Profile 引用，供 status、logs 和 fetch 动态读取。之后更新 endpoint 只影响新作业，
不会把历史作业请求误发到新机器。
...
3. 执行 `connection test autodl-main` 验证认证信息、host 和 port。
